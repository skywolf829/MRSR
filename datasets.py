...
class NetworkDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if((z_end-z_start) / self.subsample_dist > self.opt['cropping_resolution']):
            z_start = torch.randint(self.opt['z_resolution'] - self.opt['cropping_resolution']*self.subsample_dist)[0]
        ...
        if((y_end-y_start) / self.subsample_dist > self.opt['cropping_resolution']):
            y_start = torch.randint(self.opt['y_resolution'] - self.opt['cropping_resolution']*self.subsample_dist)[0]
        ...
        if((x_end-x_start) / self.subsample_dist > self.opt['cropping_resolution']):
            x_start = torch.randint(self.opt['x_resolution'] - self.opt['cropping_resolution']*self.subsample_dist)[0]
